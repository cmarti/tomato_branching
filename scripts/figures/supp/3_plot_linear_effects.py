...
def pivot_gt_data(gt_data, cols1, cols2, use_mle=True):
    ...
    if use_mle:
        gt_data = gt_data.loc[
            (gt_data["saturated_upper"] - gt_data["saturated_lower"])
            < np.log(1e3),
            :,
        ]
        df = pd.pivot_table(
...
if __name__ == "__main__":
    ...
    for use_mle, figlabel in [(True, "S6"), (False, "S7")]:
        ...
        # Init figure
        print(
            "Plotting effects of mutations across backgrounds for Figure {}".format(
                figlabel
            )
        )
        ...
        show_legend = True
        for allele, ax_row, allele_label in zip(
            EJ2_SERIES, subplots.transpose(), EJ2_SERIES_LABELS
        ):
            ej2_labels = {
                "W": "$EJ2$",
                "H": allele_label + "/+",
        ...
        fig.supylabel(
            "branching events in mutant background",
            fontsize=7,
            x=0.025,
            y=0.52,
            ha="center",
